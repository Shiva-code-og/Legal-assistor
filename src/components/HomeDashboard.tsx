    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between">
        ...
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center space-x-2 bg-emerald-500/20 border border-emerald-500/30 px-3 py-1 rounded-full text-emerald-400 text-xs font-medium mb-4">
          ...
          <p className="text-slate-300 text-sm leading-relaxed">
            Your consumer rights guardian is active. Aegis Engine is currently monitoring your active cases and ready to audit new disputed bills or statements.
          </p>
        </div>
        ...
        <div className="mt-6 md:mt-0 relative z-10 shrink-0">
          <button
            ...
            className="px-6 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg shadow-blue-600/30 flex items-center space-x-2 transition-all transform hover:-translate-y-0.5"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: "Total Cases", value: totalCases, icon: FolderKanban, change: "+2 this month", color: "text-blue-600", bg: "bg-blue-50" },
          { title: "Recovered Amount", value: recoveredAmount, icon: DollarSign, change: "Est. Total Value", color: "text-emerald-600", bg: "bg-emerald-50" },
          { title: "Pending Cases", value: pendingCases, icon: Clock, change: "In active workflow", color: "text-amber-600", bg: "bg-amber-50" },
          { title: "Completed Cases", value: completedCases, icon: CheckCircle2, change: "Successfully settled", color: "text-indigo-600", bg: "bg-indigo-50" },
        ].map((stat, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs font-medium text-slate-500 mt-1">{stat.change}</div>
              </div>
            </motion.div>
          );
        })}
          ...
          <div className="space-y-4">
            {cases.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                ...
                <button
                  onClick={() => setActiveTab("new-case")}
                  className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-medium hover:bg-blue-700 transition-colors"
                >
                  Create Case
                </button>
              </div>
              ...
              cases.slice(0, 3).map((c) => (
                <div
                  ...
                  className="p-4 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/20 transition-all cursor-pointer flex items-center justify-between"
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className="block text-[10px] font-medium text-emerald-600 mt-0.5">Win Prob: {c.confidence}%</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <div className="space-y-6 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
              {defaultRecentActivities.map((act) => (
                <div key={act.id} className="relative flex items-start space-x-4">
                  <div className="w-3 h-3 rounded-full bg-blue-600 ring-4 ring-white relative z-10 mt-1" />
                  <div>
                    <h4 className="text-xs font-semibold text-slate-900">{act.title}</h4>
                    <span className="text-[11px] text-slate-600">{act.time}</span>
                    <span className="block text-[11px] font-mono text-emerald-600 mt-0.5">Value: {act.amount}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-8 pt-4 border-t border-slate-100">
            <button
              ...
              className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition-colors flex items-center justify-center space-x-2"
            >
